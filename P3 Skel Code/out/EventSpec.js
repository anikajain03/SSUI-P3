...
export class EventSpec {
    ...
    bindRegion(regionList) {
        ...
        // **** YOUR CODE HERE ****
        // using name property of the region to check if found
        for (const region of regionList) {
            if (this._regionName === region.name) {
                // bind region if found
                this._region = region;
                return;
            }
        }
    ...
    match(evtType, regn) {
        // **** YOUR CODE HERE ****
        // matching any region case
        if (this._region === undefined && this._regionName === "*") {
            return true;
        }
        // Check if the event types match
        if (this._evtType === evtType) {
            // match if regions are the same
            if (this._region) {
                return this._region === regn;
            }
            // match regardless of region
            return true;
        }
        // no match found
        return false;
