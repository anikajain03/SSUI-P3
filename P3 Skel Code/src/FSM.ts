...
export class FSM {
    ...
    public damage() : void {
        ...
        // **** YOUR CODE HERE ****
        // pass damage notification to parent FSM 
        if (this._parent) {
            this._parent.damage();
        }
    ...
    protected _finalize() : void {
        ...
        for (const state of this._states) {
            for (const transition of state.transitions) {
                transition.bindTarget(this._states); // bind target state for each transition
            }
        }
    ...
    public actOnEvent(evtType : EventType, reg? : Region) {
        ...
        for (let transition of this.currentState.transitions) {
            if (transition.match(evtType, reg)) {
                // execute all actions with transition
                for (let action of transition.actions) {
            ...
            return; // ensures event is "consumed"
        }
    }
