...
export class Transition {
    ...
    public match(evtType : EventType, regn? : Region) : boolean {
        ...
        // **** YOUR CODE HERE ****
        // Use the onEvent specification to check if the event matches
    ...
    public bindTarget(stateList : readonly State[]) : void {
        ...
        // **** YOUR CODE HERE ****
        // Look for the target state in the provided list
        for (const state of stateList) {
            // bind if found, otherwise generate error
            //console.log(this.targetName, state.name);
            if (this.targetName === state.name) {
                this._target = state;
                return;
            }
        }   
